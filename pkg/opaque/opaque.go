...
func ServerRegisterInit(AgentID uuid.UUID, o opaque.Opaque, key kyber.Scalar) (opaque.Opaque, *Server, error) {
	...
	if !bytes.Equal(userRegInit.UserID, agentIDBytes) {
		slog.Debug("OPAQUE Server Registration Init", "OPAQUE UserID", fmt.Sprintf("%X", userRegInit.UserID), "Fox3 Message UserID", fmt.Sprintf("%X", agentIDBytes))
		regUUID, _ := uuid.FromBytes(userRegInit.UserID)
		return opaque.Opaque{}, &server, fmt.Errorf("the OPAQUE UserID %s doesn't match the Fox3 message ID %s", regUUID, AgentID)
	}
...
func ServerRegisterComplete(AgentID uuid.UUID, o opaque.Opaque, server *Server) (opaque.Opaque, error) {
	...
	// Check to make sure Fox3 UserID matches OPAQUE UserID
	if !bytes.Equal(agentIDBytes, server.regComplete.UserID) {
		return opaque.Opaque{}, fmt.Errorf("the OPAQUE UserID: %v doesn't match the Fox3 UserID: %v", server.regComplete.UserID, agentIDBytes)
	}
