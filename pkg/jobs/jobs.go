...
type Info struct {
	...
	output    string    // The job result output (stdout+stderr)
}
...
// Output returns the job result output
func (i *Info) Output() string {
	return i.output
}

// SetOutput stores the job result output
func (i *Info) SetOutput(s string) {
	i.output = s
}
