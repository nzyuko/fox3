...
// SMB listener's do not have a server because the Fox3 Server does not send/receive messages. They are sent through
...
import (
	...
	"github.com/nzyuko/fox3/v2/pkg/transformer/encoders/base64"
...
type Listener struct {
	...
	pipe         string                       // pipe is the full UNC path of the named pipe used for communications (e.g., \\.\pipe\Fox3)
...
func NewSMBListener(options map[string]string) (listener Listener, err error) {
	...
	/*
		...
		if len(temp) < 5 {
			err = fmt.Errorf("A full UNC named pipe path was not provided (e.g., \\\\.\\pipe\\Fox3)")
			return
		}
		// If the UNC path is \\.\pipe\Fox3 than it is on the local host
		// If the UNC path is \\192.168.10.11\pipe\Fox3 then validate the IP address
		if temp[1] != "." {
...
// Server is not used by UDP listeners because the Fox3 Server itself does not listen for or send Agent messages.
