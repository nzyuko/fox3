...
var jobsIn = make(chan fox3Job.Job, 1000)

// JobsOut is a channel used by the pkg/server/jobs/jobs.go to send data to the Fox3 agent
...
type Connection struct {
	...
	Conn net.Conn
	// Index is used to track and order data being sent to the SOCKS client (atomic for concurrent access)
	Index atomic.Int32
}
...
// CleanupAgent tears down all SOCKS resources for a removed agent:
// closes the listener, signals done, and closes all per-agent connections.
func CleanupAgent(agentID uuid.UUID) {
	// Stop listener
	if l, ok := listeners.LoadAndDelete(agentID); ok {
		done.Store(agentID, true)
		_ = l.(net.Listener).Close()
	}
	done.Delete(agentID)

	// Close all connections belonging to this agent
...
func processMessage() {
	for {
		job := <-jobsIn
		agent := job.AgentID
		socks, ok := job.Payload.(fox3Job.Socks)
		if !ok {
			slog.Error("processMessage: unexpected job payload type, expected Socks", "Agent", agent)
			continue
		}
		...
		currentIndex := conn.(*Connection).Index.Load()
		if int(currentIndex) == socks.Index {
			n, err := conn.(*Connection).Conn.Write(socks.Data)
			conn.(*Connection).Index.Add(1)
			if err != nil {
			...
			if core.Debug {
				slog.Debug(fmt.Sprintf("Received job out of order for agent %s connection %s. Expected %d, got %d", agent, socks.ID, currentIndex, socks.Index))
			}
			jobsIn <- job
		}
	}
...
func In(job fox3Job.Job) {
	...
	socks, ok := job.Payload.(fox3Job.Socks)
	if !ok {
		slog.Error("socks.In: unexpected job payload type, expected Socks")
		return
	}
