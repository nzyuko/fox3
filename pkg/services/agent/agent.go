...
import (
	...
	"github.com/nzyuko/fox3/v2/pkg/agents/memory"
	"github.com/nzyuko/fox3/v2/pkg/group"
	groupMemory "github.com/nzyuko/fox3/v2/pkg/group/memory"
...
func (s *Service) Remove(id uuid.UUID) (err error) {
	...
	// Clean up all per-agent resources
	hvnc.Unregister(id)
	push.Unregister(id)
	socks.CleanupAgent(id)
	rportfwd.CleanupAgent(id)
	slog.Info(fmt.Sprintf("Removed Agent %s and cleaned up resources", id))
	return
}
