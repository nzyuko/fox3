...
import (
	...
	messageMemory "github.com/nzyuko/fox3/v2/pkg/client/message/memory"
	"github.com/nzyuko/fox3/v2/pkg/core"
	"github.com/nzyuko/fox3/v2/pkg/delegate"
	delegateMemory "github.com/nzyuko/fox3/v2/pkg/delegate/memory"
	"github.com/nzyuko/fox3/v2/pkg/listeners"
	"github.com/nzyuko/fox3/v2/pkg/listeners/http"
	httpMemory "github.com/nzyuko/fox3/v2/pkg/listeners/http/memory"
	"github.com/nzyuko/fox3/v2/pkg/listeners/smb"
	smbMemory "github.com/nzyuko/fox3/v2/pkg/listeners/smb/memory"
	"github.com/nzyuko/fox3/v2/pkg/listeners/tcp"
	tcpMemory "github.com/nzyuko/fox3/v2/pkg/listeners/tcp/memory"
	"github.com/nzyuko/fox3/v2/pkg/listeners/udp"
	udpMemory "github.com/nzyuko/fox3/v2/pkg/listeners/udp/memory"
...
func (s *Service) Construct(msg messages.Base) (data []byte, err error) {
	...
	slog.Debug("Construct: serializing", "agent", msg.ID, "msg_type", msg.Type)
...
func (s *Service) Handle(id uuid.UUID, data []byte) (rdata []byte, err error) {
	...
	slog.Debug("Handle: step1 UpdateStatusCheckin", "agent", id)
	err = s.agentService.UpdateStatusCheckin(a.ID(), time.Now().UTC())
	if err != nil {
		slog.Error(fmt.Sprintf("pkg/service/message.Handle(): %s", err))
	}
	slog.Debug("Handle: step2 processing message type", "agent", id, "type", msg.Type)
	...
	case messages.CHECKIN:
		// Nothing to do
	case messages.JOBS:
		slog.Debug("Handle: step3 entering jobService.Handler", "agent", id)
		err = s.jobService.Handler(msg.Payload.([]jobs.Job))
		if err != nil {
			slog.Error(fmt.Sprintf("pkg/service/message.Handle(): %s", err))
			return
		}
		slog.Debug("Handle: step4 jobService.Handler done", "agent", id)
	default:
	...
	if s.agentService.IsChild(id) {
		slog.Info("Handle: skipping getBase for child agent", "agent", id)
		return nil, nil
	}
	slog.Debug("Handle: step5 calling getBase", "agent", id, "msg_type", msg.Type)
	rdata, err = s.getBase(id)
	slog.Debug("Handle: step6 getBase done", "agent", id, "rdata_len", len(rdata), "error", err)
	return
}
...
func (s *Service) getBase(id uuid.UUID) (data []byte, err error) {
	...
	if len(returnJobs) > 0 {
		// Count job types for debug
		typeCounts := make(map[string]int)
		for _, rj := range returnJobs {
			typeCounts[rj.Type.String()]++
		}
		slog.Debug("getBase: returning jobs", "agent", id, "total", len(returnJobs), "types", typeCounts)
	}
