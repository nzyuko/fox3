...
- Upgraded packages
 - `github.com/nzyuko/fox3-message` to v1.3.0
 - `github.com/quic-go/quic-go` to v0.40.1
 - `google.golang.org/grpc` to v1.60.0
- Modified Dockerfile
  - Upgraded `ne0nd0g/fox3-base` v1.5.0 in Dockerfile
...
- [Issue 148](https://github.com/nzyuko/fox3/issues/148) - Validate & encode gRPC messages to UTF-8
...
- gRPC Server for [Fox3 CLI](https://github.com/nzyuko/fox3-cli)
  - Default interface/port is `127.0.0.1:50051` and default password is `fox3`
  - `-addr` the address to listen for gRPC connections from the Fox3 CLI
  - `-password` the password for CLI RPC clients to connect to the Fox3 server
  ...
  - `-tlsCert` TLS certificate file path for the Fox3 server
  - `-tlsKey` TLS private key file path for the Fox3 server
...
- New SMB/UDP/TCP listeners
  ...
  - Listeners DO NOT bind to interfaces/ports on the Fox3 server, used for Agent configuration/generation ONLY
...
- GitHub Actions for building and testing the Fox3 Agent
...
- Moved the `build` string out of `main.go` and into `pkg/fox3.go`
- Removed PWNBoard from `main.go`
- Removed wiki documentation from `docs` to a new repository at https://github.com/nzyuko/fox3-documentation
...
- Replaced `github.com/nzyuko/fox3/pkg/messages` with `github.com/nzyuko/fox3-message`
  - Removes the need to depend on or import the Fox3 Server package

### Removed

- Removed the interactive CLI menu to [Fox3 CLI](https://github.com/nzyuko/fox3-cli)
  - Uses gRPC to communicate with the Fox3 server

## 1.5.1 - 2023-03-08

### Added

- Documentation for proxying Agent DLL provided by @HuskyHacks in [PR 136](https://github.com/nzyuko/fox3/pull/136)

### Security

- [PR 137](https://github.com/nzyuko/fox3/pull/137) - Bump golang.org/x/text from 0.3.7 to 0.3.8 by dependabot
- [PR 138](https://github.com/nzyuko/fox3/pull/138) - Bump golang.org/x/net from 0.0.0-20220708220712-1185a9018129 to 0.7.0 by dependabot
- [PR 139](https://github.com/nzyuko/fox3/pull/139) - Bump golang.org/x/crypto from 0.0.0-20200709230013-948cd5f35899 to 0.1.0 by dependabot
- [PR 140](https://github.com/nzyuko/fox3/pull/140) - Bump golang.org/x/net from 0.1.0 to 0.7.0 by dependabot
...
- Added new SOCKS5 functionality
  - Use `socks start 9050` from the agent menu to start a SOCKS5 listener on port 9050 on the Fox3 server
...
- [Issue 129](https://github.com/nzyuko/fox3/issues/129) - Added references to Docker image source
...
- Server shows error when BOTH the agent and the server were not obfuscated with the SAME seed
  - [fox3-agent/15](https://github.com/nzyuko/fox3-agent/issues/15)

## 1.4.1 - 2022-04-12

### Fixed

- [Issue 126](https://github.com/nzyuko/fox3/issues/126) - Upgraded [quic-go](https://github.com/lucas-clemente/quic-go/) to v0.27.0 for Go 1.18 support
...
- Dockerfile now uses [fox3-base](https://hub.docker.com/r/ne0nd0g/fox3-base) image

## 1.2.1 - 2022-01-10

### Fixed

- [fox3-agent/Issue 6](https://github.com/nzyuko/fox3-agent/issues/6) - Message padding is now a random length instead of a fixed length
...
* [issue 116](https://github.com/nzyuko/fox3/issues/116) - Fixed missing statement to enable verbose & debug output for the server component
* [issue 119](https://github.com/nzyuko/fox3/issues/116) - Add specific agent ID instead of broadcast "F" agent ID

## 1.1.0 - 2021-08-5

### Added

- Incorporated a lot of changes by [r00t0v3rr1d3](https://github.com/r00t0v3rr1d3) & [deviousbanana](https://github.com/deviousbanana) from their [fork](https://github.com/r00t0v3rr1d3/fox3/tree/dev)
- Ctrl-C and DEL key won't exit the server without a confirmation prompt from https://github.com/r00t0v3rr1d3/fox3/commit/d5aafdb97b50e43ae9842d8f91659f65d18f60d4
- `ifconfig`/`ipconfig`: Prints host network adapter information. Windows hosts use API calls to get extra info (e.g., DHCP) from https://github.com/r00t0v3rr1d3/fox3/commit/42a12af99610e439721cbd095a2d55523e7cbc94
- Agent struct and agent `info` command contain `Process` name from https://github.com/r00t0v3rr1d3/fox3/commit/cbf875427123e6a58a528d0e38a692c2308f09c9 
...
- Replaced the `exit` command with `quit` to instruct the Fox3 server to quit running
...
- [Issue 107](https://github.com/nzyuko/fox3/issues/107) - RunModule() tasking uses `run` instead of removed `cmd`

## 1.0.0 - 2021-04-18

### Changed

- Added `main.go` to project root so that the Fox3 server is compiled when retrieved with `go get`
...
- Removed all programs from `cmd` (e.g., `cmd/fox3agent`) to their own GitHub repository
- Removed User functions from opaque package to the Agent codebase
- Removed `Invoke-Melrin` and `fox3.js` from codebase completely
...
- Added `killdate`, `maxretry`, `useragent`, `padding`, `skew` to Fox3 Agent command line arguments and Makefile
...
- Added Mythic client so [Fox3](https://github.com/MythicAgents/fox3) agent can be used with the [Mythic](https://github.com/its-a-feature/mythic) Framework

### Changes

- All commands typed into the `Fox3»` are immediately parsed with [go-shellwords](https://github.com/mattn/go-shellwords)
...
- Restructured Fox3 Agent code into multiple go files
- Created generic Fox3Client interface to facilitate using different protocols
...
- [Pull 97](https://github.com/nzyuko/fox3/pull/97) - Incorrectly validated the module and agent platforms when the agent was set to "all"

## 0.9.0 - 2020-09-13

### Added

- Incorporated a fork of `github.com/CUCyber/ja3client` allowing the Fox3 agent to establish TLS connections from a JA3 signature
...
- Created new `fox3Client` interface to handle abstract http clients and changed the Agent.Client to that interface
...
- Added new Listeners menu to create or remove multiple agent listeners per running Fox3 Server instance
...
- [Issue 88](https://github.com/nzyuko/fox3/issues/88) - Set option will now take all arguments after a space
- Added module type to LinEnum, MimiPenguin, and SwapDigger
- [Issue 71](https://github.com/nzyuko/fox3/issues/71) - Run a module against all agents
...
- Removed all command line arguments from Fox3 Server executable in favor of run-time Listeners menu
...
- [Issue 77](https://github.com/nzyuko/fox3/issues/77) - Set server and agent log files with `0640` permissions
...
- Added Fox3 server identification tool dubbed PRISM
...
- Added PRISM application to detect a Fox3 server
- Added in CLI option to set HTTP Host header for domain fronting
- [Pull 72](https://github.com/nzyuko/fox3/pull/72) - Added a Linux module that will hide the Fox3 process from tools that walk the /proc filesystem by @ForensicITGuy
- [Pull 73](https://github.com/nzyuko/fox3/pull/74) - Added 5 Linux evasion and persistence modules by @ForensicITGuy
- [Pull 76](https://github.com/nzyuko/fox3/pull/76) - Prompt user to confirm exit or quit by [Daniel Roberson](https://github.com/droberson)

### Fixed

- [Pull 74](https://github.com/nzyuko/fox3/pull/74) - Fixed issued that would crash the agent if the skew was set to 0 by @alexbires
- [Issue 54](https://github.com/nzyuko/fox3/issues/54) - Prompt to exit server when user types "quit" or "exit"
...
-[Issue 69](https://github.com/nzyuko/fox3/issues/69) - Error with modules missing type definition
-[Issue 70](https://github.com/nzyuko/fox3/issues/70) - Error parsing a shellcode file containing raw bytes

## 0.7.0 - 2019-04-11

### Added
- [Pull 48](https://github.com/nzyuko/fox3/pull/48) - Added `ls` command to agent & agent menu. Uses go code native to Fox3 instead of executing `ls` or `dir` binary
- New `NativeCmd` message struct for commands native to Fox3
- [Pull 49](https://github.com/nzyuko/fox3/pull/49) - Added tests for agent package and new `/test` directory for test HTTP server
- [Pull 50](https://github.com/nzyuko/fox3/pull/50) - Adds a killdate option to agents
  - Added `set killdate` to Fox3 Server agent menu and information table so that it can be changed from the server.
- [Pull 51](https://github.com/nzyuko/fox3/pull/51) - Adds a Go native `minidump` module for Windows agents to create and receive a minidump file of a specified process (requires elevation)
...
- [Pull 58](https://github.com/nzyuko/fox3/pull/58) - Added feature to generate in-memory TLS certificates one is not provided
  - Adds new `pkg/utils/tls.go` package to generate TLS certificates
  - Updated Fox3 server log to include certificate information
  - Test case for TLS certificate generation
- [Pull 60](https://github.com/nzyuko/fox3/pull/60) - Added `pwd` and `cd` command to agent & agent menu. 
  - Uses go code native to Fox3 instead of executing system binaries or access the system's CLI.
- fox3.dll contains a DllMain function and a function named Magic to instantiate a Fox3 agent
    - DllMain does nothing
- Added exported function named Fox3 to fox3agentdll/main.go to take in a C *char and convert to Go string
...
- [Pull 64](https://github.com/nzyuko/fox3/pull/64) - Added in Dockerfile by Alex Flores (@audibleblink)

### Fixed
- [Pull 57](https://github.com/nzyuko/fox3/pull/57) - Resolved broken JSON does not increment failedCheckin counter on Fox3 agent

### Changed
- Changed SHA1 library to SHA256 and checks from http2.go for publicly distributed Fox3 Server test certificate
- Moved execute-shellcode functionality to pkg/modules/shellcode
- fox3.c contains DllMain and exported Magic function
- Fox3 Server uses exit code 0 after printing usage information

### Removed
- Removed publicly distributed certificates from repository see [Pull 58](https://github.com/nzyuko/fox3/pull/58).
- Removed "main" and "ReflectiveLoader" functions from fox3.dll/fox3.c
...
- Added Fox3 version number and new agent checkins to Fox3 Server log file

### Fixed
- Formatting error placing time stamp and message in wrong spot in the Fox3 Server log file
...
### Added
- [Pull 45](https://github.com/nzyuko/fox3/pull/45) - Added a module for Python-based SOCKS proxying (@klusitc)
...
- [Pull 43](https://github.com/nzyuko/fox3/pull/43) - Gives users the ability to dynamically
assign the callback URL variable at compile time by setting the URL= var in the make command by Alex Flores (@audibleblink)

### Fixed
- [Pull 44](https://github.com/nzyuko/fox3/pull/44) Fixes case-sensitive autocompletion of `agent`
  on the module menu (@audibleblink)

## 0.6.5 - 2019-01-10

### Fixed
- [Issue 41](https://github.com/nzyuko/fox3/issues/41) Error when downloading a file using a path containing a space
...
### Changed
- [Issue 26](https://github.com/nzyuko/fox3/issues/26) Updated Makefile to remove .dmg file extension for Darwin
- [Issue 23](https://github.com/nzyuko/fox3/issues/23) Error when using upload command without specific arguments
- [Issue 28](https://github.com/nzyuko/fox3/issues/28) Updated MimiPenguin module to use new path to download mimipenguin.sh
- Modified `cmd/fox3agentdll/main.go` Global variable from `URL` to `url` for consistency across all packages
...
### Fixed
- [Issue 23](https://github.com/nzyuko/fox3/issues/23) Error writing an agent log entry when agent was set to 'all'
- Out of range error when setting module options in Run function

## 0.6.1 - 2018-08-11
### Added
- Added Fox3 Cheat Sheet to docs/Fox3CheatSheet.pdf
...
### Changed
- Make file extracts version number from pkg/fox3.go
...
- Added check for publicly distributed Fox3 x.509 testing certificate
...
- Updated fox3.js agent to use new embeded SysInfo struct
- Updated Make file to output files in folder of the build number
- Updated Fox3AgentDLL main.go functions to use correct params
- Fox3 Server listens on 127.0.0.1 by default
...
- cmd/fox3agent/main.go into several packages and struct in
pkg/agent/main.go
...
- Fox3 JavaScript Agent
- Fox3 JavaScript Agent test page in `data/html`
...
- Execute commands on the host operating system from Fox3 prompt
- Execute module on "all" agents

### Changed
- Menu system from one long command to multiple menus
- Fox3 Server from 1 large file to multiple library files in `pkg`
- Make file instructions for Fox3 version number
...
This release marks a stable BETA version of Fox3. Both the server and
...
download Fox3_v0.1Beta.zip and unzip the contents. Next, download the
applicable binary for your platform (i.e. fox3server_windows_x64.exe)
and place it in the root of that unzipped folder. The binary can be run
from the command line. Alternatively, Fox3 can be run directly as a go
 script with go run cmd\fox3server.go.
